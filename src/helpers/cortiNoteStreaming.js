...
class CortiNoteStreaming {
  ...
  _optionsCompatible(options) {
    ...
    if (!!this.connectionOptions.multichannel !== !!options.multichannel) {
      return false;
    }
    const prevChannels = this.connectionOptions.multichannel ? 2 : 1;
    const nextChannels = options.multichannel ? 2 : 1;
    return (
      buildAudioFormatString({ ...this.connectionOptions, channels: prevChannels }) ===
      buildAudioFormatString({ ...options, channels: nextChannels })
    );
  }
  ...
  async _createInteraction(client) {
    // The SDK's serializer requires real Date objects on
    // encounter.period.{startedAt,endedAt} — passing ISO strings fails
    // validation. endedAt == startedAt at creation marks the encounter as
    // already wrapped up, so we don't need an explicit close call on disconnect.
    const now = new Date();
  ...
  _buildStreamConfiguration(options) {
    ...
    // Stereo with channel 0 = local mic, channel 1 = system audio mirrors
    // Corti's telehealth pattern: the SDK gets per-channel speaker attribution
    // for free and we don't need voice-based diarization between the two
    // sources. Mono path keeps diarization so multiple in-room speakers on the
    // mic still get separated.
    const channels = multichannel ? 2 : 1;
    const audioFormat = buildAudioFormatString({ ...options, channels });

    // The SDK's typed StreamConfigTranscription only exposes primaryLanguage,
    // isDiarization, isMultichannel, and participants, but the serializer
    // passes through unrecognized keys. automaticPunctuation, spokenPunctuation,
    // and formatting are honored by /streams.
    const transcription = {
      primaryLanguage: language,
    };

    // Diarization runs alongside multichannel so each segment carries both a
    // channel index (mic vs system) AND a per-channel speakerId (separating
    // multiple participants on the same channel — e.g., several remote folks
    // in the system-audio capture).
    transcription.isDiarization = true;
    if (multichannel) {
      transcription.isMultichannel = true;
      transcription.participants = [
        { channel: 0, role: "multiple" },
        { channel: 1, role: "multiple" },
      ];
    } else {
      transcription.participants = [{ channel: 0, role: "multiple" }];
    }
  ...
  async connect(options = {}) {
    ...
    this.connectionOptions = {
      ...
      channels: options.multichannel ? 2 : options.channels,
    ...
    switch (message?.type) {
      case "transcript": {
        ...
        for (const segment of segments) {
          ...
          if (!text) continue;
          const channel = segment.participant?.channel ?? 0;
          ...
          if (segment.final) {
            ...
            this.onPartialTranscript?.(text, { channel, speakerId });
          }
        }
        break;
      }
...
module.exports = CortiNoteStreaming;
